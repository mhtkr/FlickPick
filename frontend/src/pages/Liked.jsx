...
const Liked = () => {
  ...
  useEffect(() => {
    const fetchLikedMovies = async () => {
      try {
        ...
        const response = await fetch("http://localhost:5000/api/user/likes", {
          headers: {
  ...
  const handleDelete = async (movieId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/user/likes/${movieId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        console.error("Failed to delete movie from likes");
      }
    } catch (error) {
      console.error("Error deleting movie from likes:", error);
    }
  };
  ...
  return (
    <div className="p-6 bg-[#0B192C] text-white min-h-screen">
      ...
      {movies.length === 0 ? (
        <div className="text-gray-500">No liked movies yet.</div>
      ) : (
